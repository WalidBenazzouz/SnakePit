class Vehicle {
  ...
  seek(target, arrival, d = 0) {
    ...
    if (arrival) {
      ...
      if (dist < this.rayonZoneDeFreinage) {
        ...
        // Si on est trop proche (distance < d), on s'arrête ou on recule pour garder la distance
        if (dist < d) {
          desiredSpeed = 0;
        }
      }
  ...
  show() {
    ...
    if (this.isHead) {
      fill(255); // White part
      circle(this.r / 2, -this.r / 2, this.r);
      circle(this.r / 2, this.r / 2, this.r);

      fill(0); // Pupils
      circle(this.r / 2 + 2, -this.r / 2, this.r / 3);
      circle(this.r / 2 + 2, this.r / 2, this.r / 3);
    }
